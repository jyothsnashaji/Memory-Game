...
export class Titles extends React.Component{
    ...
    shuffle(){
        ...
        while(j<sources.length)   
            ...
            images.splice(Math.floor(Math.random() * (images.length-1)),0,Math.floor(Math.random() * (sources.length)));   
           //  images=  [...Array(15)].map(() => Math.floor(Math.random() * (sources.length)))
            }
        
        if(images.length%num_of_images_per_row)
        {
            images=images.concat([...Array(num_of_images_per_row-images.length%num_of_images_per_row)].map(() => Math.floor(Math.random() * (sources.length))));
        }
